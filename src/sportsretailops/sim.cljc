(ns sportsretailops.sim
  ...
  escalates regardless of phase), then a product-safety-concern flag
  ...
  into the permanently-excluded recall/certification-finalization
  scope."
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "product-safety-coordinator-1"}} {:thread-id tid :resume? true}))
  ...
  (let [db (store/seed-db)
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 18 :returns 1 :stock-count-delta -19
                                          :fitting-sessions 3}} coordinator-phase-1)]
      (println r)
    ...
    (println (exec-op actor "t2" {:op :log-sales-record :store-id "store-1"
                                  :patch {:units-sold 12 :returns 0 :stock-count-delta -12
                                          :fitting-sessions 2}} coordinator-phase-3))
    ...
    (println (exec-op actor "t3" {:op :schedule-staffing-operation :store-id "store-1"
                                  :patch {:shift "weekend-fitting-bay" :date "2026-07-20" :window "10:00-18:00"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "ski-boot restock" :quantity 40 :estimated-cost 380.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "climbing-wall equipment set" :quantity 4 :estimated-cost 4200.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human retail store coordinator reviews & approves --")
      (println (approve! actor "t5")))

    (println "\n== flag-safety-concern store-1 (ALWAYS escalates, even at phase 3) ==")
    (let [r (exec-op actor "t6" {:op :flag-safety-concern :store-id "store-1"
                                 :patch {:concern "reported binding-release failure on rental snowboard batch SN-4471, one minor injury logged" :confidence 0.92}} coordinator-phase-3)]
      (println r)
      (println "-- human retail store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "import ski equipment" :quantity 15 :estimated-cost 300.0
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (println (exec-op actor-direct "t10" {:op :schedule-staffing-operation :store-id "store-1"
                                           :patch {:shift "weekday-fitting-bay" :date "2026-07-22"}} coordinator-phase-3)))

    (println "\n== log-sales-record store-1, advisor drifts into recall/certification-finalization scope -> HARD hold, permanent ==")
