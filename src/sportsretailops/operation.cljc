(ns sportsretailops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (SportsRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the SportsRetailGovernor (:govern)
  ...
  human store operator/product-safety coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [sportsretailops.advisor :as advisor]
            [sportsretailops.governor :as governor]
            [sportsretailops.phase :as phase]
...
(defn build
  ...
  `sportsretailops.store/Store`).
  opts:
    :advisor      -- a `sportsretailops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SportsRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; SportsRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
