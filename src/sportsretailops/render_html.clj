(ns sportsretailops.render-html
  ...
  (`sportsretailops.operation` -> `sportsretailops.governor` ->
  `sportsretailops.store`) through a scenario adapted from this repo's
  own `sportsretailops.sim` demo driver (`clojure -M:dev:run`, confirmed
  ...
  `vendor-2` -- ids that DO match `sportsretailops.store/demo-data`, so
  it was safe to reuse rather than author from scratch), trimmed to a
  representative subset (clean auto-commits, always-escalate ops that a
  human then approves, and five distinct HARD-hold reasons including
  vendor-unverified) and rendered deterministically -- no invented
  ...
  (:require [jp-go-dds.skin]
            ...
            [sportsretailops.advisor :as advisor]
            [sportsretailops.store :as store]
            [sportsretailops.operation :as op]
            [langgraph.graph :as g]))

(def ^:private coordinator
  {:actor-id "coord-1" :actor-role :retail-store-coordinator :phase 3})
...
(defn run-demo!
  ...
  auto-commit, including fitting-sessions which are routine for sporting
  equipment), a staffing-operation schedule for the fitting bay
  (auto-commit), a low-cost supply-order coordination naming verified
  vendor-1 (auto-commit), a high-cost supply-order (ALWAYS escalates --
  approved), and a product-safety-concern flag (ALWAYS escalates --
  approved); store-99 HARD-holds as unregistered; store-3 HARD-holds as
  registered-but-unverified; a supply-order naming unverified vendor-2
  HARD-holds on vendor-unverified; a staffing proposal whose advisor
  injects `:effect :commit` HARD-holds on effect-not-propose; a sales-
  record proposal that drifts into recall-compliance / equipment-safety-
  certification finalization scope HARD-holds permanently. Every HARD
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "t1-sales" {:op :log-sales-record :store-id "store-1"
                             :patch {:units-sold 18 :returns 1 :stock-count-delta -19
                                     :fitting-sessions 3}})

    (exec! actor "t2-staff" {:op :schedule-staffing-operation :store-id "store-1"
                             :patch {:shift "weekend-fitting-bay" :date "2026-07-20"
                                     :window "10:00-18:00"}})

    (exec! actor "t3-supply-low" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "ski-boot restock" :quantity 40
                                          :estimated-cost 380.0
                                          :vendor-id "vendor-1"}})

    (exec! actor "t4-supply-high" {:op :coordinate-supply-order :store-id "store-1"
                                   :patch {:item "climbing-wall equipment set" :quantity 4
                                           :estimated-cost 4200.0
                                           :vendor-id "vendor-1"}})
    (approve! actor "t4-supply-high")

    (exec! actor "t5-flag" {:op :flag-safety-concern :store-id "store-1"
                            :patch {:concern "reported binding-release failure on rental snowboard batch SN-4471, one minor injury logged"
                                    :confidence 0.92}})
    ...
    (exec! actor "t8-vendor" {:op :coordinate-supply-order :store-id "store-1"
                              :patch {:item "import ski equipment" :quantity 15
    ...
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
      ...
      (exec! actor-direct "t9-effect" {:op :schedule-staffing-operation :store-id "store-1"
                                       :patch {:shift "weekday-fitting-bay" :date "2026-07-22"}}))
...
(def ^:private action-gate-rows
  ...
  ;; (README Ops / Features, `sportsretailops.governor`/`sportsretailops.phase`) --
  ...
  ["        <tr><td><code>:log-sales-record</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence (sale/return/fitting session logging)</span></td></tr>"
   ...
   "        <tr><td><code>:flag-safety-concern</code></td><td><span class=\"warn\">ALWAYS human approval &middot; never auto at any phase &middot; surfaces concern only, never finalizes recall or equipment-safety certification</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-4763 &middot; specialized-sporting-equipment-retail</title><style>"
     (jp-go-dds.skin/dds+skin)
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Retail sale of sporting equipment in specialized stores (ISIC 4763) — Operator Console</h1>\n"
     ...
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Specialized sporting-equipment retail stores</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>sportsretailops.store</code> via <code>sportsretailops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated from the real actor stack.</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (Sports Retail Governor)</h2>\n"
     "    <p class=\"muted\">HARD holds cannot be overridden: store unverified (registration + retail license), vendor unverified (supply-order counterparty), effect not <code>:propose</code>, and permanently out-of-scope recall-compliance / equipment-safety-certification finalization territory. An unregistered or unverified store, or an unverified vendor, never reaches a human.</p>\n"
     "    <table>\n"
