(ns sportsretailops.phase
  "Phase 0->3 staged rollout for the ISIC-4763 'Retail sale of sporting
  equipment in specialized stores' operations-coordination actor.
  ...
  rollout milestone still to come. Flagging a product-safety concern
  always needs a human to actually look at it.
  `sportsretailops.governor`'s own `always-escalate-ops` enforces the
  same invariant independently -- two layers, not one, agree on this."
  (:require [sportsretailops.governor :as governor]))
...
(defn verdict->disposition
  "Map a SportsRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
