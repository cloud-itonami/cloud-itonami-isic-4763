(ns sportsretailops.store
  "SSoT for the ISIC-4763 'Retail sale of sporting equipment in
  ...
  sporting-goods storefront: inventory/sale/return/fitting transaction
  logging (bike, ski/snowboard, golf and other equipment fitting
  sessions are routine here, unlike general-merchandise retail), floor-
  staff scheduling, sporting-equipment supply-order coordination with
  registered vendors, and product-safety-concern flagging (suspected
  recall-eligible batches, defective equipment, injury reports). It
  never finalizes a recall-compliance decision, never issues or revokes
  an equipment-safety certification, and never itself makes a
  product-safety-compliance determination -- see
  `sportsretailops.governor`'s `scope-exclusion-violations`, a HARD,
  ...
  commit or escalate -- `sportsretailops.governor`'s
...
(defn demo-data
  ...
  []
  {:stores
   {"store-1" {:store-id "store-1" :name "Trailhead Sporting Goods"
               :registered? true :verified? true}
    "store-2" {:store-id "store-2" :name "Riverside Ski & Board Shop"
               :registered? true :verified? true}
    "store-3" {:store-id "store-3" :name "Downtown Pop-Up Cycle Kiosk (in intake)"
               :registered? true :verified? false}}
   :vendors
   {"vendor-1" {:vendor-id "vendor-1" :name "Alpine Gear Wholesale"
                :registered? true :verified? true}
    "vendor-2" {:vendor-id "vendor-2" :name "Unverified Import Sports Co."
                :registered? true :verified? false}}})
