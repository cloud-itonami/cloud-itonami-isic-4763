(ns sportsretailops.governor-test
  "Pure unit tests of `sportsretailops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [sportsretailops.advisor :as adv]
            [sportsretailops.governor :as gov]
            [sportsretailops.store :as store]))

(def store-1 {:store-id "store-1" :name "Trailhead Sporting Goods" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Cycle Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Alpine Gear Wholesale" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Sports Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine sporting-goods store coordination"
...
(deftest vendor-check-is-scoped-to-supply-order-only
  (testing "non-supply-order ops never trip :vendor-unverified, even with no vendors registered at all"
    (let [s (store/mem-store {"store-1" store-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :flag-safety-concern]]
...
(deftest op-outside-allowlist-is-hard
  (testing "an op outside the closed four-op allowlist is a scope violation"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (clean-proposal :finalize-recall-compliance "store-1") s)]
      (is (true? (:hard? verdict)))
      (is (some #{:op-not-allowed} (map :rule (:violations verdict)))))))

(deftest recall-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly issuing a product recall is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "issued the recall for the defective batch and notified all affected customers"
                          :confidence 0.99)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest certification-grant-finalization-content-is-hard
  (testing "a proposal touching granting a safety certification is HARD-blocked, same as a recall"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "granted the safety certification for the climbing-harness batch before review completed"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest certification-revoke-content-is-hard
  (testing "a proposal touching revoking a safety certification is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-staffing-operation "store-1")
                          :summary "revoked the safety certification for the treadmill line without further review")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest declared-compliant-content-is-hard
  (testing "a proposal touching declaring equipment compliant is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "declared the equipment compliant at the loading dock before inspection")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-safety-concern-is-not-scope-excluded
  (testing "flagging an observed recall/defective-equipment/injury concern as a SAFETY CONCERN (not a finalized recall/certification decision) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-safety-concern "store-1")
                         :value {:concern "reported binding-release failure on rental snowboard batch SN-4471, one minor injury logged"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defective equipment/injury/recall suspicion) is exactly what this op exists to surface"))))
...
(deftest safety-concern-always-escalates-clean
  (testing ":flag-safety-concern is always high-stakes/escalate, even when otherwise clean and high confidence"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (assoc (clean-proposal :flag-safety-concern "store-1") :confidence 0.99) s)]
...
(deftest low-cost-supply-order-does-not-force-escalate
  (testing "a :coordinate-supply-order at or below the cost threshold does not trip the high-cost escalate gate"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          cheap (assoc (clean-supply-order "store-1" "vendor-1" 380.0) :confidence 0.9)
...
;; "recall" or "certification"), which then accidentally matches inside
    ...
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-staffing-operation :coordinate-supply-order
        ...
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "ski-boot restock" :estimated-cost 380.0 :vendor-id "vendor-1"}
                      {})
