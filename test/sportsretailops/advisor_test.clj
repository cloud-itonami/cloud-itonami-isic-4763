(ns sportsretailops.advisor-test
  "Unit tests of `sportsretailops.advisor` proposal generation."
...
(deftest propose-safety-concern-shape
  (testing "safety-concern proposal always escalates"
    (let [p (adv/infer db {:op :flag-safety-concern
                           :store-id "store-1"
                           :patch {:concern "binding-release failure on rental snowboard batch"}})]
